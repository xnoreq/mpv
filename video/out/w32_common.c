...
static void DropTarget_Destroy(DropTarget* This) {
    ...
    talloc_free(This->iface.lpVtbl);
    This->iface.lpVtbl = NULL;
    talloc_free(This);
}

static HRESULT STDMETHODCALLTYPE DropTarget_QueryInterface(IDropTarget* This, REFIID riid, void** ppvObject) {
...
static HRESULT STDMETHODCALLTYPE DropTarget_Drop(IDropTarget* This, IDataObject* pDataObj, DWORD grfKeyState, POINTL pt, DWORD* pdwEffect) {
    ...
    if (pDataObj->lpVtbl->GetData(pDataObj, &fmtetc_file, &medium) == S_OK) {
        if (GlobalLock(medium.hGlobal) != NULL) {
            ...
            char** files = talloc_array(NULL, char*, numFiles);
            ...
            mp_event_drop_files(t->vo->input_ctx, numFiles, files);
    ...
    else if (pDataObj->lpVtbl->GetData(pDataObj, &fmtetc_url, &medium) == S_OK) {
        // get the URL encoded in US-ASCII
        char* url = (char*)GlobalLock(medium.hGlobal);
        if (url != NULL) {
            if (mp_event_drop_mime_data(t->vo->input_ctx, "text/uri-list", bstr0(url)) > 0) {
...
static void DropTarget_Init(DropTarget* This, struct vo *vo) {
    IDropTargetVtbl* vtbl = talloc(NULL, IDropTargetVtbl);
