...
static int parse_channels(radio_priv_t* priv,float freq_channel,float* pfreq){
    ...
    if (priv->radio_param->channels){
        ...
        MP_INFO(priv, "[radio] Radio channel names detected.\n");
        ...
        while (*channels) {
            ...
            if ((priv->radio_channel_current->freq>priv->rangehigh)||(priv->radio_channel_current->freq<priv->rangelow))
                MP_ERR(priv, "[radio] Wrong frequency for channel %s\n",
        ...
        if (priv->radio_channel_current->index!=channel){
            if (((float)((int)freq_channel))!=freq_channel)
                MP_ERR(priv, "[radio] Wrong channel number: %.2f\n",freq_channel);
            else
                MP_ERR(priv, "[radio] Wrong channel number: %d\n",(int)freq_channel);
            return STREAM_ERROR;
        }
        MP_INFO(priv, "[radio] Selected channel: %d - %s (freq: %.2f)\n", priv->radio_channel_current->index,
            priv->radio_channel_current->name, priv->radio_channel_current->freq);
        *pfreq=priv->radio_channel_current->freq;
    }else{
        if (freq_channel){
            MP_INFO(priv, "[radio] Radio frequency parameter detected.\n");
            priv->radio_channel_list=malloc(sizeof(radio_channels_t));
            priv->radio_channel_list->next=NULL;
            priv->radio_channel_list->prev=NULL;
            priv->radio_channel_list->index=1;
    ...
    MP_DBG(priv, "[radio] Done parsing channels.\n");
    return STREAM_OK;
}
...
static int init_frac_v4l2(radio_priv_t* priv){
    ...
    if (ioctl(priv->radio_fd, VIDIOC_G_TUNER, &tuner)<0){
        MP_WARN(priv, "[radio] Warning: ioctl get tuner failed: %s. Setting frac to %d.\n",strerror(errno),priv->frac);
        return  STREAM_ERROR;
    }
    if(tuner.type!=V4L2_TUNER_RADIO){
        MP_ERR(priv, "[radio] %s is no radio device!\n",priv->radio_param->device);
        return STREAM_ERROR;
    }
    if(tuner.capability & V4L2_TUNER_CAP_LOW){
        priv->frac=16000;
        MP_DBG(priv, "[radio] tuner is low:yes frac=%d\n",priv->frac);
    }
    else{
        priv->frac=16;
        MP_DBG(priv, "[radio] tuner is low:no frac=%d\n",priv->frac);
    }

    priv->rangelow=((float)tuner.rangelow)/priv->frac;
    priv->rangehigh=((float)tuner.rangehigh)/priv->frac;
    MP_VERBOSE(priv, "[radio] Allowed frequency range is %.2f-%.2f MHz.\n",priv->rangelow,priv->rangehigh);
    return STREAM_OK;
}
...
static int set_frequency_v4l2(radio_priv_t* priv,float frequency){
    ...
    if(ioctl(priv->radio_fd,VIDIOC_S_FREQUENCY,&freq)<0){
        MP_ERR(priv, "[radio] ioctl set frequency 0x%x (%.2f) failed: %s\n",freq.frequency,
...
static int get_frequency_v4l2(radio_priv_t* priv,float* frequency){
    ...
    if (ioctl(priv->radio_fd, VIDIOC_G_FREQUENCY, &freq) < 0) {
        MP_ERR(priv, "[radio] ioctl get frequency failed: %s\n",strerror(errno));
        return  STREAM_ERROR;
    }
...
static void set_volume_v4l2(radio_priv_t* priv,int volume){
    ...
    if (ioctl(priv->radio_fd, VIDIOC_S_CTRL, &control)<0){
        MP_WARN(priv, "[radio] ioctl set mute failed: %s\n",strerror(errno));
    }

    memset(&qctrl,0,sizeof(qctrl));
    qctrl.id = V4L2_CID_AUDIO_VOLUME;
    if (ioctl(priv->radio_fd, VIDIOC_QUERYCTRL, &qctrl) < 0) {
        MP_WARN(priv, "[radio] ioctl query control failed: %s\n",strerror(errno));
        return;
    }
    ...
    if (ioctl(priv->radio_fd, VIDIOC_S_CTRL, &control) < 0) {
        MP_WARN(priv, "[radio] ioctl set volume failed: %s\n",strerror(errno));
    }
}
...
static int get_volume_v4l2(radio_priv_t* priv,int* volume){
    ...
    qctrl.id = V4L2_CID_AUDIO_VOLUME;
    if (ioctl(priv->radio_fd, VIDIOC_QUERYCTRL, &qctrl) < 0) {
        MP_ERR(priv, "[radio] ioctl query control failed: %s\n",strerror(errno));
        return STREAM_ERROR;
    }
    ...
    if (ioctl(priv->radio_fd, VIDIOC_G_CTRL, &control) < 0) {
        MP_ERR(priv, "[radio] ioctl get volume failed: %s\n",strerror(errno));
        return STREAM_ERROR;
    }
...
static inline int set_frequency(radio_priv_t* priv,float frequency){
    if ((frequency<priv->rangelow)||(frequency>priv->rangehigh)){
        MP_ERR(priv, "[radio] Wrong frequency: %.2f\n",frequency);
        return STREAM_ERROR;
    }
    ...
    if(clear_buffer(priv)!=STREAM_OK){
        MP_ERR(priv, "[radio] Clearing buffer failed: %s\n",strerror(errno));
        return  STREAM_ERROR;
    }
#endif
   return STREAM_OK;
...
static int init_audio(radio_priv_t *priv)
{
    ...
    priv->do_capture=1;
    MP_VERBOSE(priv, "[radio] Starting capture stuff.\n");
#if HAVE_ALSA
    ...
    if(audio_in_init(&priv->audio_in, priv->log, is_oss?AUDIO_IN_OSS:AUDIO_IN_ALSA)<0){
        MP_ERR(priv, "[radio] audio_in_init failed.\n");
    }
    ...
    if (audio_in_setup(&priv->audio_in) < 0) {
        MP_ERR(priv, "[radio] audio_in_setup call failed: %s\n", strerror(errno));
        return STREAM_ERROR;
    }
#if HAVE_OSS_AUDIO
    if(is_oss)
...
int radio_set_freq(struct stream *stream, float frequency){
    ...
    MP_INFO(priv, "[radio] Current frequency: %.2f\n",frequency);
    return 1;
}
...
int radio_step_channel(struct stream *stream, int direction) {
    ...
    if (priv->radio_channel_list) {
        switch (direction){
            case  RADIO_CHANNEL_HIGHER:
                ...
                if(!radio_set_freq(stream,priv->radio_channel_current->freq))
                    return 0;
                MP_VERBOSE(priv, "[radio] Selected channel: %d - %s (freq: %.2f)\n",
        ...
        MP_ERR(priv, "[radio] Can not change channel: no channel list given.\n");
    return 1;
}
...
int radio_set_channel(struct stream *stream, char *channel) {
    ...
    if (*channel=='\0')
        MP_ERR(priv, "[radio] Wrong channel name: %s\n",channel);
...
static int open_s(stream_t *stream,int mode)
{
    ...
    MP_VERBOSE(priv, "[radio] Available drivers: ");
    ...
    if(priv->driver)
        ...
        MP_INFO(priv, "[radio] Unknown driver name: %s\n",priv->radio_param->driver);
        close_s(stream);
        return STREAM_ERROR;
    }
    ...
    if ((frequency<priv->rangelow)||(frequency>priv->rangehigh)){
        ...
        MP_INFO(priv, "[radio] Using frequency: %.2f.\n",frequency);
